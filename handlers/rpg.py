...
from utils import safe_edit_text, get_progress_bar, calculate_level_stats
from keyboards.menu import get_main_menu, get_lot_kb
...
async def show_menu(message: Message):
    await message.answer("Welcome to NOTAPES Ecosystem!", reply_markup=get_main_menu())


@router.callback_query(F.data == "menu_main")
async def show_menu_cb(callback: CallbackQuery):
    await safe_edit_text(
        callback.message, "Welcome to NOTAPES Ecosystem!", reply_markup=get_main_menu()
    )
...
async def show_stats(callback: CallbackQuery):
    ...
    if not user:
        await callback.answer("You need to send some messages first!", show_alert=True)
        return
    ...
    await safe_edit_text(callback.message, text, reply_markup=get_main_menu())
    await callback.answer()
...
async def show_burn(callback: CallbackQuery):
    ...
    if not lots.data:
        await callback.answer("No secret lots available right now!", show_alert=True)
        return
...
async def process_burn(callback: CallbackQuery):
    ...
    if success:
        await callback.answer(
            "🔥 XP Burned! You've claimed the secret lot!", show_alert=True
        )
